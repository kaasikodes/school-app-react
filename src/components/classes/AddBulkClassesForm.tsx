...
import { useAddDepartmentInBulk } from "../../helpersAPIHooks/departments";
import { GlobalContext } from "../../contexts/GlobalContextProvider";
import { ISaveDepartmentInBulkProps } from "../../helpers/department";
import { useQueryClient } from "react-query";
import { useAuthUser } from "react-auth-kit";
import { IAuthDets } from "../../appTypes/auth";
import { openNotification } from "../../helpers/notifications";
import {
  downloadBulkClassesUploadTemplate,
  ISaveClassInBulkProps,
} from "../../helpers/classes";
import { useAddClassesInBulk } from "../../helpersAPIHooks/classes";
...
const AddBulkClassesForm: React.FC<IProps> = ({ closeDrawer }) => {
  ...
  const handleChange: UploadProps["onChange"] = (
    ...
    reader.onload = function (e: any) {
      try {
        ...
        if (columns[0] !== "name" || columns[1] !== "description") {
          throw new Error(
        ...
        const formattedData = retrievedData.map((item, entryIndex) => {
          let ans: any = {};
          columns.forEach((col, i) => {
            ans[col] = item[i] ? item[i] : null;
            if (col === "name" && ans[col] === null) {
              throw new Error(
                `A class must have a name, check the entry ${entryIndex + 2}`
              );
            }
          });
  ...
  const { mutate, isLoading } = useAddClassesInBulk();

  const handleSubmit = (e: React.MouseEvent) => {
    if (schoolId) {
      const props: ISaveClassInBulkProps = {
        schoolId,
        token,
    ...
    <div className="flex flex-col gap-6">
      <div className="border border-dotted border-slate-500 rounded flex flex-col items-center gap-2 py-3 px-2">
        ...
        <Typography.Text title="Please Download template and populate">
          <a href={downloadBulkClassesUploadTemplate()}>
...
export default AddBulkClassesForm;
