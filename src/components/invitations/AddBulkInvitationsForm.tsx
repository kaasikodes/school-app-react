...
import { downloadBulkInvitationsUploadTemplate } from "../../helpers/invites/bulkInvitationsTemplate";
...
export const AddBulkInvitationsForm: React.FC<IProps> = ({ closeDrawer }) => {
        ...
        const formattedData = retrievedData.map((item, entryIndex) => {
          ...
          columns.forEach((col, i) => {
            ...
            if (col === "name" && ans[col] === null) {
              throw new Error(
                `A department must have a name, check the entry ${
                  entryIndex + 2
                }`
              );
            }
  ...
  const { mutate, isLoading } = useAddDepartmentInBulk();

  const handleSubmit = (e: React.MouseEvent) => {
    if (schoolId) {
      const props: ISaveDepartmentInBulkProps = {
        schoolId,
        token,
    ...
    <div className="flex flex-col gap-6">
      <div className="border border-dotted border-slate-500 rounded flex flex-col items-center gap-2 py-3 px-2">
        ...
        <Typography.Text title="Please Download template and populate">
          <a href={downloadBulkInvitationsUploadTemplate()}>
