...
const SchoolSessionCoursesTable = ({ searchTerm }: IProps) => {
  ...
  const columns = [
    {
      ...
      render: (_: string, record: any) => (
        <Link to={`/classes/3/courses/${record.id}`}>{record.name}</Link>
      ),
    },
    {
      title: "Active",
                ...
                },
                {
                  ...
                  label: (
                    <button
                      className="w-full text-left"
                      onClick={() =>
                        handleAssignTeacher(record.id, record.name)
                      }
                    >
...
export default SchoolSessionCoursesTable;
